...
# a main offline ninacatcoind, does most of the tests
# a restricted RPC ninacatcoind setup with RPC payment
# two local online ninacatcoinds connected to each other
N_ninacatcoinDS = 5

# 4 wallets connected to the main offline ninacatcoind
# 1 wallet connected to the first local online ninacatcoind
# 1 offline wallet
N_WALLETS = 7
...
ninacatcoind_base = [builddir + "/bin/ninacatcoind", "--regtest", "--fixed-difficulty", str(DIFFICULTY), "--no-igd", "--p2p-bind-port", "ninacatcoind_p2p_port", "--rpc-bind-port", "ninacatcoind_rpc_port", "--zmq-rpc-bind-port", "ninacatcoind_zmq_port", "--zmq-pub", "ninacatcoind_zmq_pub", "--non-interactive", "--disable-dns-checkpoints", "--check-updates", "disabled", "--rpc-ssl", "disabled", "--data-dir", "ninacatcoind_data_dir", "--log-level", "1", "--rpc-max-connections-per-private-ip", "100", "--rpc-max-connections", "100"]
ninacatcoind_extra = [
  ["--offline"],
...
wallet_base = [builddir + "/bin/ninacatcoin-wallet-rpc", "--wallet-dir", WALLET_DIRECTORY, "--rpc-bind-port", "wallet_port", "--rpc-ssl", "disabled", "--daemon-ssl", "disabled", "--log-level", "1", "--allow-mismatched-daemon-version"]
wallet_extra = [
...
for i in range(N_ninacatcoinDS):
  command_lines.append([str(18180+i) if x == "ninacatcoind_rpc_port" else str(18280+i) if x == "ninacatcoind_p2p_port" else str(18380+i) if x == "ninacatcoind_zmq_port" else "tcp://127.0.0.1:" + str(18480+i) if x == "ninacatcoind_zmq_pub" else builddir + "/functional-tests-directory/ninacatcoind" + str(i) if x == "ninacatcoind_data_dir" else x for x in ninacatcoind_base])
  if i < len(ninacatcoind_extra):
    command_lines[-1] += ninacatcoind_extra[i]
  outputs.append(open(FUNCTIONAL_TESTS_DIRECTORY + '/ninacatcoind' + str(i) + '.log', 'a+'))
