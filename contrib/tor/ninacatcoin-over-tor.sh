...
for dir in \
  ...
  "$DIR/../../build/Windows/master/release/bin"
do
  if test -x "$dir/ninacatcoind"
  then
    ninacatcoind="$dir/ninacatcoind"
    break
  fi
done
if test -z "$ninacatcoind"
then
  echo "ninacatcoind not found"
  exit 1
fi
echo "Found: $ninacatcoind"

TORDIR="$DIR/ninacatcoin-over-tor"
TORRC="$TORDIR/torrc"
...
"$ninacatcoind" \
  ...
  --add-priority-node 2nianode5itf65lz.onion:18083 \
  --detach
ready=0
for i in `seq 10`
do
  sleep 1
  status=$("$ninacatcoind" status)
...
if test "$ready" = 0
then
  echo "Error starting ninacatcoind"
  tail -n 400 "$HOME/.ninacatcoin/ninacatcoin.log" | grep -Ev stacktrace\|"Error: Couldn't connect to daemon:"\|"src/daemon/main.cpp:.*ninacatcoin\ \'" | tail -n 20
  exit 1
fi
