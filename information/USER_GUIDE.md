...
✅ **Monitors network health** (hashrate, difficulty, checkpoints)  
...
- **CPU:** Runs in daemon threads, minimal overhead
- **Memory:** <50 MB (static variables in RAM)
- **Network:** Only P2P connections (not internet)
- **Disk:** Minimal - logs integrated with daemon logging
...
1. Detect tampering via SHA-256 hash comparison
2. Alert via Discord webhook (if configured)
3. Attempt auto-remediation (download + recompile)
4. If remediation fails 3 times, quarantine node

Nota: El proceso de remediación genera comandos y reportes;
la descarga y recompilación dependen de la configuración del nodo.
