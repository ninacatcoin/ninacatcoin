# The Current/Future Status of ZMQ in Ninacatcoin

## ZMQ Pub/Sub
Client `ZMQ_SUB` sockets must "subscribe" to topics before it receives any data.
This allows filtering on the server side, so network traffic is reduced. Ninacatcoin
...
matching supported by both Ninacatcoin and ZMQ. The `format`, `context` and `event`
...
The Ninacatcoin daemon will ensure that events prefixed by `chain` will be sent in
...
chain instead of the last block. The Ninacatcoin daemon also ensures that
...
The Ninacatcoin daemon will send a `txpool_add` pub exactly once for each
