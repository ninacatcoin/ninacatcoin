# Anonymity Networks with Ninacatcoin

Currently only Tor and I2P have been integrated into Ninacatcoin. The usage of
...
is made or until `ninacatcoind` is shutdown and restarted with only public
connections enabled.

Anonymity networks can also be used with `ninacatcoin-wallet-cli` and
`ninacatcoin-wallet-rpc` - the wallets will connect to a daemon through a proxy. The
...
Ninacatcoind does not support synchronizing the blockchain over onion or I2P hidden services.
...
`--tx-proxy` which tells `ninacatcoind` the ip/port of a socks proxy provided by a
...
which tells `ninacatcoind` to connect to ".onion" P2P addresses using a socks
...
`--anonymous-inbound`. This option tells `ninacatcoind` the inbound address, network
...
which tells `ninacatcoind` that a max of 25 inbound Tor connections are being
received at address "5tymba6faziy36md5ffy42vatbjzlye4vyr3gyz6lcvdfximnvwpmwqd.onion:18084" and forwarded to `ninacatcoind`
...
forwarded to `ninacatcoind` localhost port 18085. Using `tx-proxy`(required), these
...
**_Note: The specified port for `anonymous-inbound` must be unique (not 19020 etc). `anonymous-inbound` is not for blockchain sync!_**
...
`ninacatcoind` RPC port - which is independent from the configuration for incoming
...
```text
HiddenServiceDir /var/lib/tor/data/ninacatcoin-rpc
...
```text
# P2P Hidden service
HiddenServiceDir /var/lib/tor/data/ninacatcoin
...
This will store key information in `/var/lib/tor/data/ninacatcoin` and `/var/lib/tor/data/ninacatcoin-rpc`
and will forward "Tor port" 18084 and 18089 to ports 18084 and 18089 of ip 127.0.0.1, respectively. The file
`/usr/lib/tor/data/ninacatcoin/hostname` will contain the ".onion" address for use with `--anonymous-inbound`, and
`/var/lib/tor/data/ninacatcoin-rpc/hostname` will contain the ".onion" address for use with RPC.

I2P must be configured with a standard server tunnel. Configuration differs by
I2P implementation.  
You can find guides for i2pd [here](https://docs.getninacatcoin.org/running-node/ninacatcoind-tori2p/#__tabbed_1_2).
...
system clock is fairly accurate - many peers on the Ninacatcoin network should have
similar timestamps.
...
### Intermittent Ninacatcoin Syncing

If a user only runs `ninacatcoind` to send a transaction then quit, this can also
be used by an ISP to link a user to a transaction.

#### Mitigation

Run `ninacatcoind` as often as possible to conceal when transactions are being sent.
...
`ninacatcoind` currently selects two outgoing connections every 5 minutes for
...
`ninacatcoind` will have rotated its selected outgoing connections several times in
...
circuits is observable by the ISP. Instead, `ninacatcoind` should likely disconnect
